...
vignette: >
  %\VignetteIndexEntry{estimate_trophic_position_two_source_model}
...
The purpose of this vignette is to learn how to estimate trophic position of a species using stable isotope data ($\delta^{13}C$ and $\delta^{15}N$). We can estimate trophic position using a two source model
that is based on equations from [Post 2002](ttps://esajournals.onlinelibrary.wiley.com/doi/abs/10.1890/0012-9658%282002%29083%5B0703%3AUSITET%5D2.0.CO%3B2) and [Vander Zaden and (2002)](https://esajournals.onlinelibrary.wiley.com/doi/full/10.1890/0012-9658%282002%29083%5B2152%3AFAIOBA%5D2.0.CO%3B2).
...
$$
 ...
 the mean isotope value of the first baseline, $\delta^{15}C_{b2}$ is the mean 
 isotope value of the second baseline. For aquatic ecosystems, often 
 ...
 does not always happen which can be problematic with [Heuvel et al. 2024](https://cdnsciencepub.com/doi/10.1139/cjfas-2024-0028) 
proposing a new 
method to correct (i.e., scale) these values. $\alpha$ is then used the equation below. 
...
different trophic positions of baseline $\lambda$. The equation replaces 
...
### Bayesian model 
To use this model with a Bayesian framework, we need to rearrange the 
...
and then the use estimates of $\alpha$ in the rearranged equation for trophic 
position. 
...
The function `two_source_model()` uses this rearranged equation. If using 
baselines from two different trophic levels, you can set `lambda` to `2` to replace
$\lambda$ (`l1`) with the mixing model equation above. 
...
You will notice that I have renamed this object to `conusmer_iso_2` this is for 
two reasons the first being that we are modifying `consumer_iso` and should make 
a new object and second is that to render these vignettes it needs them to 
be new objects. 
...
by 0 and 1 and assumes a beta distribution, $\Delta N$ 
assumes a normal distribution (`dn`; $\mu = 3.4$; $\sigma = 0.25$), 
...
that corrects (i.e., scales) this equation providing better and more meaningful
estimates.
...
that corrects or scales $\alpha$ appropriately using [Heuvel et al. 2024](https://cdnsciencepub.com/doi/10.1139/cjfas-2024-0028).
...
We rename `d15n_tp_Intercept` to `tp`, add the grouping columns, round all columns that are numeric to two decimal points using `mutate_if()`, and rearrange the order of the columns using `dplyr::select()`.
...
demonstrated in another vignette (i.e, [one source - multiple groups](estimate_trophic_position_one_source_multiple_groups.html)).
