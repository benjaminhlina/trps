...
When assessing the model we want R-hat to be 1 or within 0.05, 
...
"grassy" or "catapillery", and posterior distributions to look relatively
normal. 
...
For this example, however, we first are going to plot the density for posterior
draws using `geom_density()` and then as a point interval using 
`stat_pointinterval()`. 
...
(e.g., comparing trophic position among species or different 
ecoergion). 
...
using a two source model but can be removed for this analysis.
...
the data into a list for all groups and then use `map()` from 
...
of `map()` here as `walk()` invisibly return `.x` which is handy when you want 
to call a function (`plot()`) for its side effects rather than its return value.
I have also added `grid.text()` from {grid} to add in the name of the group
to each plot. 
...
You will notice that `"b_tp_Intercept"` is the name of the variable in the 
model object that we are most interested in extracting. We then get posterior 
draws using `gather_draws()`. We are going to rename `"b_tp_Intercept"` to `tp`. 
...
[{dplyr}](https://dplyr.tidyverse.org/). The variable `name` will have the ecoregion and common name 
of the species, however, they will pasted to together by a `_`. We need 
separate the the string into the two variables we want `ecoregion` and 
`common_name`. We can do this by using `separate_wider_delim()` from 
[{tidyr}](https://tidyr.tidyverse.org/). When using this function it will separate the columns and 
