...
post_draws
```
We can see that this consists of seven variables, `ecoregion`,
`common_name`, `.chain`,
`.iteration`(number sampled after burn-in), 
`.draw` (number sampled from `iter`; adjusted using `thin`), 
`.variable` (this will have different variables dependig on what is 
supplied to `gather_draws()`), `.value` (estimated value). 
