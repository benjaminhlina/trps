...
(`common_name`), the ecoregion samples were collected from (`ecoregion`), and 
...
`group_by()`. Lastly, we will use `dplyr::select()` to rearrange the order of the 
columns. 
...
calculate the mean $\delta^{13}C$ and $\delta^{15}N$ for the 
baseline and add in the constant $\lambda$ (`l1`) to our data frame. 
We will do this by `groub_by()` our two groups, then using `mutate()` and `mean()` to calculate 
the mean values. 
