...
frame. We will do this by using `groub_by()`to group the data by our two
groups, then using `mutate()` and `mean()` to calculate the mean values.
...
When assessing the model we want $\hat R$ to be 1 or within 0.05 of 1,
which indicates that the variance among and within chains are equal (See
[{rstan} documentation on
$\hat R$](https://mc-stan.org/rstan/reference/Rhat.html)), a high value for
...
We can see that $\hat R$ is 1 meaning that variance among and within
chains are equal (See [{rstan} documentation on
$\hat R$](https://mc-stan.org/rstan/reference/Rhat.html)) and that ESS is
...
using `gather_draws()`. We are going to rename `"b_tp_Intercept"` 
to `tp`.
...
We can see that $\hat R$ is 1, meaning that the variance among and
within chains are equal (See [{rstan} docmentation on
$\hat R$](https://mc-stan.org/rstan/reference/Rhat.html)) and that ESS is
quite large for both groups. Overall, this means that both models are
...
instead of `map()`, as `iwalk()` invisibly return `.x` which is
...
[{tidybayes}](http://mjskay.github.io/tidybayes/) to do this work. First
we will look at the the names of the variables we want to extract using
`get_variables()`. Considering we have multiple models in `m1` that all
...
For your data you will likley have other catogory names then `ecoregion`
...
Again, because `m1` is a `list` of our model objects, we need to `map()` 
over the list to calculate these values. Then we will do the same procedures 
we have done before to combine and restructure the outputs. 
