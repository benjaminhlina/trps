...
values for both baselines (`c1`-`n2`). These columns are useful when estimating trophic position 
using a two source model but are not need for this analysis and can be removed.
...
combined_iso_update <- combined_iso %>% 
 dplyr::select(-c(d13c_b2, d15n_b2, c1, c2, n2)) %>% 
 mutate(
  l1 = 2, 
...
to call a function (e.g., `plot()`) for its side effects rather than its returned value.
...
$\Delta N$ is estimated to be `3.89` with `l-95% CI` of `2.91`, 
and `u-95% CI` of `3.86`. If we move down to trophic position (`tp`) we 
...
We will use functions from [{tidybayes}](http://mjskay.github.io/tidybayes/) to do this work. First we will look at 
the the names of the variables we want to extract using `get_variables()`. 
Considering we have multiple models in `m1`, need to use `map()` to iterate
over `m1` to get variables.
...
model object that we are most interested in extracting. We then will gather
 posterior draws using `gather_draws()`. We are going to rename 
...
Lastly, let's look at the point interval plot but first we need to create the 
colour palette. 
