...
calculate the mean $\delta^{13}C$ and $\delta^{15}N$ and 
add in the constant $\lambda$ (`l1`) to our data frame. We ill do this by 
`groub_by()` our two groups, then using `mutate()` and `mean()` to calculate 
the mean values. 
```{r}
...
to increase the amount of samples that are burned-in (discarded; in `brm()` 
...
argument `iter`), increase the the number of samples drawn (in `brm()` 
this is controled by the argument `thin`), and/or adjust the `adapt_delta` 
value using `control = list(adapt_delta = 0.95)`. 

When assessing the model we want R-hat to be 1 or within 0.05 of 1, 
meaning that variance among and within chains is equal 
(See 
...
`common_name`, `.chain` (chain number), 
`.iteration`(number sampled after burn-in; adjusted using `inter`), 
`.draw` (number sampled from `iter`; adjusted using `thin`), 
`.variable` (this will have different variables of interest if you 
...
baselines. These columns are useful when estimating trophic position 
...
Let's remove the columns we don't need, `d13c_b2`, `d15n_b2`, `c2`, `n2`. 
...
to call a function (e.g., `plot()`) for its side effects rather than its return value.
