---
title: "estimate_trophic_position_two_source_model"
...
$$
 ...
 method to correct (i.e., scale) these values.
 
 $alpha$ is then used the equation below. 
...
There is a variation of this model that using a mixing model to consider 
different tropic positions of baseline $\lambda$. The equation replaces 
...
is from above. Only use this replace for $\lambda$ if you have baselines from 
two different trophic levels. 
...
$$
\delta^{13}C_c = \alpha * (\delta^{13}C_{b1} - \delta^{13}C_{b2}) + 
\delta^{13}C_{b2}
$$
...
and upper bound = 10), and if informed priors are desired for $\delta^{15}N_{b1}$ 
and  $\delta^{15}N_{b2}$ (`n1` and `n2`; $\mu = 8$ and $9.5$; $\sigma = 1$), 
$\delta^{153N_b$ (`n1` and `n2`; $\mu = 8$ and $9.5$; $\sigma = 1$), 
